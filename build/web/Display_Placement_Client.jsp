...
<html>
    ...
    <body>
        <div id="brandname">
            <h1> Aristan CRM - Placement Client Entry Details </h1>
      ...
      <a href="Placement_management.jsp">
    ...
    try{
        ...
        while(rs.next())
        {
            ...
            <tr>
                ...
                <td> <%=rs.getString("COMPANY_URL")%> </td>
                <td> <%=rs.getString("FIRST_NAME")%> </td>
                <td> <%=rs.getString("LAST_NAME")%> </td>
                <td> <%=rs.getString("CONTACT_NO")%> </td>
                <td> <%=rs.getString("EMAIL_ID")%> </td>
                <td> <%=rs.getString("TECH_REQUIREMENT")%> </td>
                <td> <%=rs.getString("REQUIRE_STATUS")%> </td>
                <td> <%=rs.getString("SHOW_DATE")%> </td>
                <td> <%=rs.getString("REMARK")%> </td>
                <td> <%=rs.getString("ADDRESS")%> </td>
              
                
                <td><a href="DeletePlacementEntry.jsp?id=<%=rs.getString("CLIENT_ID")%>">
                     <span class="glyphicon glyphicon-trash" onclick="return confirm('Do you want  to delete the record ? ')"></span></a></td>
               
               <td><a href="UpdatePlacementEntry.jsp?id=<%=rs.getString("CLIENT_ID")%>">
